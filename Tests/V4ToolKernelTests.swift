...
final class V4ToolKernelTests: XCTestCase {
    ...
    func testNotesStepInfersAppendArguments() async {
        let tool = KernelTestTool(
            ...
            schema: V4ToolInputSchema(
                fields: [
                    ...
                    V4ToolInputField(name: "targetTitle", kind: .string, isRequired: false, summary: "targetTitle"),
                    V4ToolInputField(name: "query", kind: .string, isRequired: false, summary: "query"),
            ...
            XCTAssertEqual(arguments["action"]?.stringValue, "append")
            XCTAssertEqual(arguments["targetTitle"]?.stringValue, "项目周报")
            XCTAssertEqual(arguments["body"]?.stringValue, "补充这段内容到项目周报备忘录")
            return V4ToolExecutionOutput(
                outputText: "ok",
                evidenceSummary: "apple.notes.create action=append; note_id=n1",
                rawPayload: .object(["action": .string("append"), "noteID": .string("n1")])
            )
        }
        let kernel = V4ToolKernel(
            registry: V4ToolRegistry(tools: [tool]),
    ...
    func testNotesStepInfersFindArguments() async {
        let tool = KernelTestTool(
            toolName: "apple.notes.create",
            schema: V4ToolInputSchema(
                fields: [
            ...
            XCTAssertEqual(arguments["action"]?.stringValue, "find")
            XCTAssertEqual(arguments["query"]?.stringValue, "项目周报")
            return V4ToolExecutionOutput(
                outputText: "ok",
                evidenceSummary: "apple.notes.create action=find; matched=1",
                rawPayload: .object(["action": .string("find"), "matchedCount": .number(1)])
            )
        }
        let kernel = V4ToolKernel(
            registry: V4ToolRegistry(tools: [tool]),
