...
struct SettingsView: View {
    ...
    private var homeProductIntroCard: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Label("告别键盘，开口就能写；不用切输入法，也不用先点输入框。", systemImage: "text.bubble")
                .font(PulseUI.Typography.body)
            Label("模型可以自由添加与切换，写作、翻译、整理都能用你习惯的那一套。", systemImage: "square.grid.2x2")
                .font(PulseUI.Typography.body)
            Label("长按魔术先生，眼前这段文字可以直接改写、翻译、精简、重排。", systemImage: "wand.and.stars")
                .font(PulseUI.Typography.body)
            Label("一口气全念对适合讨论、脑暴和短会，一次讲完就给你结构化结果。", systemImage: "brain.head.profile")
                .font(PulseUI.Typography.body)
            Label("时光机把提醒与时间线放在同一处，关键事项有节奏地往前推进。", systemImage: "clock.badge.exclamationmark")
                .font(PulseUI.Typography.body)
        }
        .frame(maxWidth: .infinity, alignment: .leading)
        .padding(14)
