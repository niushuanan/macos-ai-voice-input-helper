        ...
        for item in searchQueries {
            let result = await runOsaScript(
                lines: [
                    ...
                    // Use Apple Music web search inside Music, then click the first matching result and press Play.
                    "set encodedText to do shell script \"python3 -c 'import urllib.parse,sys; print(urllib.parse.quote(sys.argv[1]))' \" & quoted form of keywordText",
                    "set searchURL to \"https://music.apple.com/search?term=\" & encodedText",
                    "do shell script \"open -a Music \\\"\" & searchURL & \"\\\"\"",
                    "delay 1.4",
                    "tell application \"System Events\"",
                    "tell process \"Music\"",
                    "set frontmost to true",
                    "set w to window 1",
                    // Wait for search results to render; Music UI trees can lag when opened from background.
                    "set targetButton to missing value",
                    "set shortText to keywordText",
                    "try",
                    "if (count of characters of keywordText) >= 2 then",
                    "set shortText to text 1 thru 2 of keywordText",
                    "end if",
                    "end try",
                    "repeat with attemptIdx from 1 to 25",
                    ...
                    // Prefer entries that include the query text. This approximates \"first result\" in the UI.
                    "if (nm as string) contains keywordText then",
                    "set targetButton to e",
                    "exit repeat",
                    "end if",
                    // Fallback: match the first 2 characters for short Chinese queries.
                    "if shortText is not \"\" then",
                    "if (nm as string) contains shortText then",
                    "set targetButton to e",
                    "exit repeat",
                    "end if",
                    "end if",
                    ...
                    "click targetButton",
                    "delay 1.0",
                    // Wait for the page to render play/pause controls. UI trees can lag, especially when Music is backgrounded.
                    "set playBtn to missing value",
                    "set sawPause to false",
                    "repeat with attemptIdx from 1 to 25",
                    "set elems2 to entire contents of window 1",
                    "repeat with e2 in elems2",
                    "try",
                    "if (role of e2) is \"AXButton\" then",
                    "set nm2 to missing value",
                    "set d2 to missing value",
                    "try",
                    "set nm2 to name of e2",
                    "end try",
                    "try",
                    "set d2 to description of e2",
                    "end try",
                    "if nm2 is not missing value then",
                    "if (nm2 as string) is \"播放\" then",
                    "set playBtn to e2",
                    "exit repeat",
                    "end if",
                    "if (nm2 as string) is \"暂停\" then",
                    "set sawPause to true",
                    "end if",
                    "end if",
                    "if d2 is not missing value then",
                    "if (d2 as string) contains \"播放\" then",
                    "set playBtn to e2",
                    "exit repeat",
                    "end if",
                    "if (d2 as string) contains \"暂停\" then",
                    "set sawPause to true",
                    "end if",
                    "end if",
                    "end if",
                    "end try",
                    "end repeat",
                    "if playBtn is not missing value or sawPause is true then exit repeat",
                    "delay 0.20",
                    "end repeat",
                    "if playBtn is missing value and sawPause is false then return \"ui_script_failed\"",
                    "if playBtn is not missing value then click playBtn",
                    "end tell",
                    "end tell",
                    "delay 0.8",
                    "tell application \"Music\"",
